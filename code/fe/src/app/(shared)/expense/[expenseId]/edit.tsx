...
export default function EditExpenseScreen() {
  ...
  const replace = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: expenseQuery(expenseId).queryKey }),
  ...
  return <Screen>{replace.error ? <ErrorMessage error={replace.error} /> : null}<ExpenseEditor currentUserId={profile.data.id} displayCurrency={profile.data.displayCurrency} expense={expense.data} members={members} saving={replace.isPending} onSave={(body) => replace.mutate({ ...body, expectedVersion: expense.data.version })} /></Screen>;
}
