...
export default function ExpenseDetailScreen() {
  ...
  if (expense.error || profile.error || !expense.data || !profile.data)
    return (
      <Screen>
        <ErrorMessage
          error={
            expense.error ?? profile.error ?? new Error('Expense not found.')
          }
        />
      </Screen>
    );
    ...
    <Screen>
      ...
      <Card>
        <Row
          ...
          subtitle={expense.data.occurredAt.slice(0, 10)}
          detail={formatMinorAmount(
            expense.data.totalMinor,
            profile.data.displayCurrency
          )}
        />
      </Card>
      {expense.data.status === 'DELETED' ? (
        <Notice title="Deleted expense">
      ...
      <Allocations
        title="PAID BY"
        items={expense.data.payers}
        displayCurrency={profile.data.displayCurrency}
        name={name}
      />
      <Allocations
        title="OWED BY"
        items={expense.data.participants.map((item) => ({
          userId: item.userId,
          amountMinor: item.owedMinor
        }))}
        displayCurrency={profile.data.displayCurrency}
        name={name}
      />
      {editable ? (
        <View className="gap-3">
          <Link
            href={{
              pathname: '/expense/[expenseId]/edit',
              params: { expenseId }
            }}
            asChild
          >
          ...
          <Button
            ...
            onPress={() =>
              Alert.alert(
                'Delete expense?',
                'This reverses its balance effect while keeping an auditable record.',
                [
...
function Allocations({
  ...
  items: { userId: string; amountMinor: string }[];
      ...
      <Card>
        {items.map((item) => (
          <Row
            ...
            detail={formatMinorAmount(item.amountMinor, displayCurrency)}
          />
        ))}
      </Card>
    </View>
