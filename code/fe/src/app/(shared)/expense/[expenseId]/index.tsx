...
function Allocations({ displayCurrency, items, name, title }: { displayCurrency: Parameters<typeof formatMinorAmount>[1]; items: { userId: string; amountMinor: string }[]; name: (userId: string) => string; title: string }) {
  return <View style={{ gap: 8 }}><SectionLabel>{title}</SectionLabel><Card>{items.map((item) => <Row key={item.userId} title={name(item.userId)} detail={formatMinorAmount(item.amountMinor, displayCurrency)} />)}</Card></View>;
}
