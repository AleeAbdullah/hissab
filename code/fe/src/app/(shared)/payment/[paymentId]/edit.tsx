...
export default function EditPaymentScreen() {
  ...
  const replace = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['ledgers', settlement.data!.ledgerId] }),
        queryClient.invalidateQueries({ queryKey: userBalancesQuery.queryKey }),
        queryClient.invalidateQueries({ queryKey: settlementQuery(paymentId).queryKey }),
  ...
  const members = balances.data.members.map((member) => ({ userId: member.userId, displayName: member.displayName }));
  ...
  return <Screen>{replace.error ? <ErrorMessage error={replace.error} /> : null}<SettlementEditor balances={balances.data} currentUserId={profile.data.id} displayCurrency={profile.data.displayCurrency} members={members} saving={replace.isPending} settlement={settlement.data} onSave={save} /></Screen>;
}
