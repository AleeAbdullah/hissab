...
export default function PaymentDetailScreen() {
  ...
  if (settlement.error || profile.error || !settlement.data || !profile.data) return <Screen><ErrorMessage error={settlement.error ?? profile.error ?? new Error('Payment not found.')} /></Screen>;
  ...
  const editable = settlement.data.status === 'ACTIVE' && settlement.data.createdByUserId === profile.data.id;
  return (
    <Screen>
      ...
      <Card><Row title={`${names.get(settlement.data.fromUserId) ?? 'Member'} paid ${names.get(settlement.data.toUserId) ?? 'member'}`} subtitle={settlement.data.occurredAt.slice(0, 10)} detail={formatMinorAmount(settlement.data.amountMinor, settlement.data.currency)} /></Card>
