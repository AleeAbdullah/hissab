...
export default function FriendDetailScreen() {
  ...
  if (connections.isLoading || profile.isLoading || balances.isLoading)
    return <Loading />;
  if (
    connections.error ||
    balances.error ||
      ...
      <Screen>
        <ErrorMessage
          error={
            ...
            balances.error ??
            profile.error ??
            new Error('Friend ledger not found.')
          }
        />
      </Screen>
    );
  ...
  return (
    <Screen>
      ...
      <LedgerActions
        draft={{
          ledgerId: friend.ledgerId,
          ledgerName: friend.displayName,
          currentUserId: profile.data.id,
          displayCurrency: profile.data.displayCurrency,
          members: [
            { userId: profile.data.id, displayName: profile.data.displayName },
