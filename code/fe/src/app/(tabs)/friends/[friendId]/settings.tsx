    ...
    <Screen>
      ...
      <View className="flex-row items-center gap-4 py-2">
        ...
        <View className="flex-1 gap-0.5">
          <Text selectable className="text-2xl font-bold leading-[30px]">{friend?.displayName ?? 'Friend'}</Text>
          <Text selectable className="text-[15px] leading-[22px] text-muted-foreground">{friend ? 'Active connection' : 'Connection details unavailable'}</Text>
        </View>
      </View>
      <Card>
        <View className="gap-1 p-4">
          <Text selectable className="text-xs font-semibold leading-4 tracking-[0.4px] text-muted-foreground">RELATIONSHIP</Text>
          <Text selectable className="leading-6">{friend ? 'Direct connection is active.' : 'Open this page from your Friends list.'}</Text>
        </View>
      </Card>
      ...
      <Button variant="destructiveOutline" disabled={!canBlock || blockFriend.isPending} accessibilityState={{ disabled: !canBlock || blockFriend.isPending, busy: blockFriend.isPending }} onPress={() => Alert.alert(`Block ${friend?.displayName ?? 'person'}?`, 'This archives the settled direct ledger. It does not delete its audit history.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Block person', style: 'destructive', onPress: () => blockFriend.mutate() }])}>{blockFriend.isPending ? <ActivityIndicator className="text-destructive" /> : <Text>Block person</Text>}</Button>
      <Link href="/friends/blocked" asChild><Button variant="outline" role="link"><Text>View blocked people</Text></Button></Link>
    </Screen>
  );
}
