...
export default function FriendsScreen() {
  ...
  const filtered = (connections.data ?? []).filter((friend) => `${friend.displayName} ${friend.email ?? ''}`.toLowerCase().includes(search.trim().toLowerCase()));
    ...
    <Screen>
        ...
        options={{
          headerRight: () => (
            <Link href="/connection-new" asChild>
              <Button variant="ghost" size="icon" role="link" accessibilityLabel="Add connection"><Text className="text-[30px] font-light text-primary">＋</Text></Button>
            </Link>
          ),
        }}
      />
      ...
      {filtered.length ? (
        ...
        <View className="items-stretch gap-3 py-6">
          ...
          {!search ? <Link href="/connection-new" asChild><Button role="link"><Text>Add connection</Text></Button></Link> : null}
        </View>
      )}
    </Screen>
  );
