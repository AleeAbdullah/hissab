...
export default function ReportsScreen() {
  ...
  const apply = () => {
    ...
    if ((fromDate && !from) || (toDate && !to) || (from && to && from >= to))
      return setValidationError('Enter a valid date range as YYYY-MM-DD.');
    setValidationError(null);
    setFilters({ from, to, bucket: filters.bucket });
  };
  if (profile.isLoading || report.isLoading) return <Loading />;
  if (profile.error || report.error || !profile.data || !report.data)
    return (
      <Screen>
        <ErrorMessage
          error={
            profile.error ??
            report.error ??
            new Error('Reports are unavailable.')
          }
        />
      </Screen>
    );
    ...
    <Screen>
      ...
      <Card>
        <Row
          title="Report mode"
          detail={
            report.data.mode === 'OWED_SHARE' ? 'Your share' : 'Cash paid'
          }
          href="/report-mode"
        />
      </Card>
      <View className="gap-2">
        ...
        <Field
          label="To"
          hint="YYYY-MM-DD, inclusive and optional"
          value={toDate}
          onChangeText={setToDate}
          autoCapitalize="none"
        />
        <Button variant="outline" onPress={apply}>
          <Text>Apply period</Text>
        </Button>
      </View>
      <View className="gap-2">
        ...
        <ChoiceChips
          ...
          onChange={(value) =>
            setFilters({
              ...filters,
              bucket: value as PersonalReport['bucket']
            })
          }
        />
      </View>
      <PersonalSummary
        ...
        report={report.data}
        title={filters.from || filters.to ? 'Selected period' : 'All time'}
      />
      <View className="gap-2">
        <SectionLabel>
          {report.data.bucket === 'MONTH' ? 'MONTHLY TOTALS' : 'DAILY TOTALS'}
        </SectionLabel>
        {report.data.buckets.length ? (
          <Card>
            {report.data.buckets
              .slice()
              .reverse()
          ...
          <Text selectable>No totals for this period.</Text>
        )}
      </View>
    </Screen>
  );
