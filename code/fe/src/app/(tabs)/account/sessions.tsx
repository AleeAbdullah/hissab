...
export default function SessionsScreen() {
  ...
  const mutation = useMutation({
    mutationFn: (sessionId?: string) =>
      sessionId ? revokeSession(sessionId) : revokeOtherSessions(),
    onSuccess: () =>
  ...
  const sessionCard = (session: NonNullable<typeof current>) => (
    <Card key={session.id}>
      <View className="gap-2 p-3">
        <Text selectable className="text-[17px] font-semibold">
        ...
        >
          Signed in {date(session.createdAt)} · Last used{' '}
          {date(session.lastUsedAt)} · Expires {date(session.expiresAt)}
        </Text>
        {!session.current ? (
          <Button
            ...
            accessibilityState={{
              disabled: mutation.isPending,
              busy: mutation.isPending && mutation.variables === session.id
            }}
            onPress={() =>
              Alert.alert(
                'Revoke this session?',
                ...
                [
                  { text: 'Cancel', style: 'cancel' },
                  {
                    text: 'Revoke',
                    style: 'destructive',
                    onPress: () => mutation.mutate(session.id)
                  }
                ]
              )
            }
          >
            {mutation.isPending && mutation.variables === session.id ? (
      ...
      {others.length ? (
        ...
        >
          {mutation.isPending && mutation.variables === undefined ? (
            ...
            <Text>Revoke all other sessions</Text>
          )}
        </Button>
      ) : (
