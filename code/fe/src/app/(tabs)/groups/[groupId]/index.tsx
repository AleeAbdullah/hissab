    ...
    <Screen>
      ...
      <Card>
        <Row
          ...
          href={{ pathname: '/groups/[groupId]/balances', params: { groupId } }}
        />
        <Row
          title="Members"
          href={{ pathname: '/groups/[groupId]/members', params: { groupId } }}
        />
        {canManage ? (
          <Row
            title="Edit group"
            href={{ pathname: '/groups/[groupId]/edit', params: { groupId } }}
          />
        ) : null}
        {canManage ? (
          <Row
      ...
      {canManage && profile.data && members.data ? (
        <LedgerActions
          draft={{
            ledgerId: groupId,
            ledgerName: group.data.name,
      ...
      {group.data.status === 'ARCHIVED' ? (
        <Notice title="Read-only group">
          Archived groups keep their history and cannot be changed.
        </Notice>
      ) : null}
      {group.data.membershipStatus === 'LEFT' ? (
        <Notice title="You left this group">
          You can still view its membership history, but cannot change it.
        </Notice>
      ) : null}
      {members.data && profile.data ? (
        <LedgerActivity
          displayCurrency={profile.data.displayCurrency}
          ledgerId={groupId}
      ...
      {profile.error || members.error ? (
        <ErrorMessage error={profile.error ?? members.error} />
      ) : null}
      <Link
        href={{ pathname: '/groups/[groupId]/balances', params: { groupId } }}
        asChild
