...
export default function GroupMembersScreen() {
  ...
  if (group.error || members.error || !group.data)
    return (
      <Screen>
        <ErrorMessage
          error={group.error ?? members.error ?? new Error('Group not found.')}
        />
      </Screen>
    );
  ...
  return (
    <Screen>
      ...
      {invitations.error || cancel.error ? (
        <ErrorMessage error={invitations.error ?? cancel.error} />
      ) : null}
      ...
      {canManage ? (
        <Link
          href={{ pathname: '/members-select', params: { groupId } }}
          asChild
        >
          <Button role="link">
                  ...
                  <View className="shrink-0">
                    <Button
                      ...
                      accessibilityState={{
                        ...
                        busy:
                          cancel.isPending &&
                          cancel.variables === invitation.userId
                      }}
                      onPress={() =>
                        Alert.alert(
                          'Cancel invitation?',
                          `Remove ${invitation.userDisplayName}'s pending invitation to ${group.data.name}?`,
                          [
                            { text: 'Keep invitation', style: 'cancel' },
                            {
                              text: 'Cancel invitation',
                              style: 'destructive',
                              onPress: () => cancel.mutate(invitation.userId)
                            }
                          ]
                        )
                      }
      ...
      {!canManage ? (
        <Notice title="Member changes unavailable">
          Only active members can invite people or cancel pending invitations.
        </Notice>
      ) : null}
    </Screen>
  );
