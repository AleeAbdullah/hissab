...
export default function GroupSettingsScreen() {
  ...
  return (
    <Screen>
      ...
      {leave.error || archive.error ? (
        <ErrorMessage error={leave.error ?? archive.error} />
      ) : null}
      {canManage ? (
        <Notice title="Settlement check">
          The backend prevents leaving or archiving while any required group
          balances remain unsettled.
        </Notice>
      ) : (
        <Notice title="Settings unavailable">
          Only active members can manage an active group.
        </Notice>
      )}
      {canManage ? (
        <Button
          ...
          disabled={pending}
          accessibilityState={{ disabled: pending, busy: leave.isPending }}
          onPress={() =>
            Alert.alert(
              'Leave group?',
              `You will lose access to ${group.data.name}'s active ledger. This cannot be undone.`,
              [
      ...
      {canManage ? (
        <Button
          variant="destructive"
          disabled={pending}
          accessibilityState={{ disabled: pending, busy: archive.isPending }}
          onPress={() =>
            Alert.alert(
              'Archive group?',
              `Archive ${group.data.name} for everyone once all group balances are settled.`,
              [
