  ...
  return (
    <Screen>
      <View className="gap-2">
        <Text selectable className="text-xl font-semibold leading-[25px]">
          Find someone
        </Text>
        <Text
          selectable
        ...
        >
          Search by the exact email address on their account. Hissab does not
          list or suggest people.
        </Text>
      </View>
      {lookup.error || send.error ? (
        <ErrorMessage error={lookup.error ?? send.error} />
      ) : null}
      <TextInput
        ...
        onSubmitEditing={() =>
          email.includes('@') && lookup.mutate(email.trim().toLowerCase())
        }
        className="min-h-12 rounded-xl bg-muted px-4 text-base leading-6 text-foreground placeholder:text-muted-foreground"
      />
      <Button
        disabled={!email.includes('@') || lookup.isPending}
        accessibilityState={{
          disabled: !email.includes('@') || lookup.isPending,
          busy: lookup.isPending
        }}
        onPress={() => lookup.mutate(email.trim().toLowerCase())}
      >
        {lookup.isPending ? (
      ...
      {lookup.isSuccess && !candidate ? (
        <Text
          selectable
          className="py-6 text-center text-[15px] leading-[22px] text-muted-foreground"
        >
          No connection is available for that email. Check the spelling or
          confirm the email with them.
        </Text>
      ) : null}
      {candidate ? (
        <ConnectionCandidateCard
          candidate={candidate}
          isSending={send.isPending}
          requestSent={send.isSuccess}
          onSend={() => send.mutate(candidate.userId)}
        />
      ) : null}
    </Screen>
  );
