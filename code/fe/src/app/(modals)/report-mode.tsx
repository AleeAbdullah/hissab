...
export default function ReportModeScreen() {
  ...
  if (query.error || !query.data)
    return (
      <Screen>
        <ErrorMessage
          error={query.error ?? new Error('Profile is unavailable.')}
        />
      </Screen>
    );
...
function ReportModeForm({ profile }: { profile: Profile }) {
  ...
  return (
    <Screen>
      <Notice title="How shared expenses are counted">
        This changes Personal reports only. It never changes anyone’s balance.
      </Notice>
      ...
      <View className="gap-2">
        ...
        <ChoiceChips
          choices={[
            { label: 'Your share', value: 'OWED_SHARE' },
            { label: 'Cash paid', value: 'CASH_OUT_OF_POCKET' }
          ]}
          value={mode}
