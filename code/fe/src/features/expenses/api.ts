...
export const expenseCategoriesQuery = {
  ...
  queryFn: () =>
    request<SharedExpenseCategory[]>(() => expensesListCategories())
};

export function listExpenses(ledgerId: string, cursor?: string) {
  return request<SharedExpensePage>(() =>
    expensesListExpenses({ path: { ledgerId }, query: { cursor } })
  );
}
...
export function expenseQuery(expenseId: string) {
  return {
    ...
    queryFn: () =>
      request<SharedExpense>(() => expensesGetExpense({ path: { expenseId } }))
  };
}

export function createExpense(ledgerId: string, body: CreateExpenseDto) {
  return request<SharedExpense>(() =>
    expensesCreateExpense({
      path: { ledgerId },
      body,
