...
export function LedgerActivity({ displayCurrency, ledgerId, members }: { displayCurrency: DisplayCurrency; ledgerId: string; members: LedgerDraftMember[] }) {
  ...
  return (
    <View className="gap-4">
      ...
      {expenses.hasNextPage ? <Button variant="outline" disabled={expenses.isFetchingNextPage} accessibilityState={{ disabled: expenses.isFetchingNextPage, busy: expenses.isFetchingNextPage }} onPress={() => expenses.fetchNextPage()}>{expenses.isFetchingNextPage ? <ActivityIndicator className="text-primary" /> : <Text>Load more expenses</Text>}</Button> : null}
      ...
      {settlements.hasNextPage ? <Button variant="outline" disabled={settlements.isFetchingNextPage} accessibilityState={{ disabled: settlements.isFetchingNextPage, busy: settlements.isFetchingNextPage }} onPress={() => settlements.fetchNextPage()}>{settlements.isFetchingNextPage ? <ActivityIndicator className="text-primary" /> : <Text>Load more payments</Text>}</Button> : null}
    </View>
  );
}
...
function ActivitySection<T>({ title, items, render }: { title: string; items: T[]; render: (item: T) => ReactNode }) {
  ...
  return <View className="gap-2"><SectionLabel>{title}</SectionLabel><Card>{items.map(render)}</Card></View>;
}
