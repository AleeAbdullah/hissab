...
export function PersonalTransactionEditor({ displayCurrency, onSave, saving, transaction }: { displayCurrency: DisplayCurrency; onSave: (body: CreatePersonalTransactionDto) => void; saving: boolean; transaction?: PersonalTransaction }) {
  const initial = transaction ? personalTransactionInitialValues(transaction) : null;
  const categories = useQuery(personalCategoriesQuery);
  const [type, setType] = useState<PersonalTransactionType>(initial?.type ?? 'EXPENSE');
  const [amount, setAmount] = useState(initial?.amount ?? '');
  const [categoryCode, setCategoryCode] = useState<PersonalCategoryCode | null>(initial?.categoryCode ?? null);
  const [description, setDescription] = useState(initial?.description ?? '');
  const [occurredDate, setOccurredDate] = useState(initial?.occurredDate ?? todayDate());
  const [merchantOrSource, setMerchantOrSource] = useState(initial?.merchantOrSource ?? '');
  const [notes, setNotes] = useState(initial?.notes ?? '');
  const [validationError, setValidationError] = useState<string | null>(null);
  const availableCategories = (categories.data ?? []).filter((category) => category.kind === type);
  const changeType = (value: PersonalTransactionType) => {
    setType(value);
    if (!(categories.data ?? []).some((category) => category.kind === value && category.code === categoryCode)) setCategoryCode(null);
  };
  const save = () => {
    const built = buildPersonalTransactionBody({ amount, categoryCode, description, merchantOrSource, notes, occurredDate, type });
  ...
  return (
    <View style={{ gap: 16 }}>
      ...
      <Field label="Description" placeholder={type === 'INCOME' ? 'e.g. Salary' : 'e.g. Groceries'} value={description} onChangeText={setDescription} />
      <Field label="Date" hint="YYYY-MM-DD" placeholder="2026-08-06" value={occurredDate} onChangeText={setOccurredDate} autoCapitalize="none" />
