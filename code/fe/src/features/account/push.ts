...
export async function registerForPushNotifications(userId: string) {
  if (!canRegisterPushNotifications()) {
    throw new Error('Push registration requires a physical iOS or Android device.');
  }

  const current = await Notifications.getPermissionsAsync();
  const permission = current.granted ? current : await Notifications.requestPermissionsAsync();
  if (!permission.granted) {
    throw new Error('Allow notifications in your device settings to turn on push notifications.');
  }
...
async function getInstallationId() {
  ...
  const id = globalThis.crypto?.randomUUID?.() ?? `hissab-${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
