        ...
        <Card>
          {items.map((item) => (
            <NotificationRow
              key={item.id}
              item={item}
              loading={markingId === item.id || isMarkingAll}
              onPress={onMarkRead}
            />
          ))}
        </Card>
      ...
      {hasNextPage ? (
        ...
        >
          {isLoadingMore ? (
            ...
            <Text>Load more notifications</Text>
          )}
        </Button>
      ) : null}
    </View>
...
function NotificationRow({
  ...
  const content = (
    <>
      <View className="flex-row items-baseline gap-2">
        ...
        {unread ? (
          <Text selectable className="text-xs font-bold leading-4 text-primary">
            Unread
          </Text>
        ) : null}
      </View>
  ...
  const className = cn(
    'min-h-[72px] gap-1 border-b border-border p-3',
    unread && 'bg-accent',
  ...
  return unread ? (
    <Button
      variant="ghost"
      accessibilityLabel={`Unread. ${item.title}. ${item.body}. ${notificationTime(item.createdAt)}`}
      accessibilityState={{ disabled: loading }}
      disabled={loading}
      onPress={() => onPress(item.id)}
      className={cn('w-full flex-col items-stretch rounded-none', className)}
    >
      {content}
    </Button>
  ) : (
