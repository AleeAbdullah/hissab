...
export function SettlementEditor({
  ...
  const [fromUserId, setFromUserId] = useState(
    initial?.fromUserId ?? currentUserId
  );
  ...
  return (
    <View className="gap-4">
      <Notice title="External payment">
        Record money already paid or received elsewhere. Hissab does not move
        money.
      </Notice>
      {validationError ? (
...
function createsCredit(
  body: CreateSettlementDto,
  balances?: LedgerBalances,
  replacing?: Settlement
) {
  const members = balances?.members;
  if (!members) return true;
  const net = new Map(
    members.map((member) => [member.userId, BigInt(member.netMinor)])
  );
  if (replacing) {
    net.set(
      replacing.fromUserId,
      (net.get(replacing.fromUserId) ?? 0n) - BigInt(replacing.amountMinor)
    );
    net.set(
      replacing.toUserId,
      (net.get(replacing.toUserId) ?? 0n) + BigInt(replacing.amountMinor)
    );
  }
  const from = net.get(body.fromUserId);
  const to = net.get(body.toUserId);
  if (from === undefined || to === undefined || from >= 0n || to <= 0n)
    return true;
  const payable = [-from, to].reduce((lowest, value) =>
