...
export const incomingGroupInvitationsQuery = {
  ...
  queryFn: () =>
    request<GroupInvitation[]>(() => groupInvitationsListIncoming())
};
...
export function groupMembersQuery(groupId: string) {
  return {
    ...
    queryFn: () =>
      request<GroupMember[]>(() => groupsListMembers({ path: { groupId } }))
  };
}
  ...
  return {
    ...
    queryFn: () =>
      request<GroupInvitation[]>(() =>
        groupsListInvitations({ path: { groupId } })
      )
  };
}
...
export function updateGroup(groupId: string, name: string) {
  return request<GroupUpdateResult>(() =>
    groupsUpdateGroup({
      path: { groupId },
      body: { name },
      headers: { 'Idempotency-Key': idempotencyKey() }
...
export function inviteGroupUser(groupId: string, userId: string) {
  return request<GroupInvitationResult>(() =>
    groupsInviteUser({
      path: { groupId },
      body: { userId },
      headers: { 'Idempotency-Key': idempotencyKey() }
...
export function acceptGroupInvitation(groupId: string) {
  return request<GroupInvitationResponse>(() =>
    groupInvitationsAccept({
      path: { groupId },
      headers: { 'Idempotency-Key': idempotencyKey() }
    })
...
export function leaveGroup(groupId: string) {
  return request<GroupLeaveResult>(() =>
    groupsLeaveGroup({
      path: { groupId },
      headers: { 'Idempotency-Key': idempotencyKey() }
    })
...
export function archiveGroup(groupId: string) {
  return request<GroupArchiveResult>(() =>
    groupsArchiveGroup({
      path: { groupId },
      headers: { 'Idempotency-Key': idempotencyKey() }
    })
