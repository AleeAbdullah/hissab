...
def build():
    ...
    password = (statusbar() + nav(title="Change password", lead=CANCEL,
                ...
                + '<div class="scroll">'
                + banner("Every other device will be signed out",
                         "Changing your password ends all sessions except this one. You will "
                         "need to sign in again on your other devices.", kind="w")
                + card('<div class="field"><span class="flbl t-cap">Current password</span>'
                       ...
                       '<span class="caret"></span></div>'
                       '<span class="fhint t-cap">At least 10 characters. Longer beats '
    ...
    sessions = (statusbar() + nav(title="Devices and sessions", back="Account")
                ...
                + listcard([
                    person_row(None, "iPhone · Safari",
                               "Karachi · signed in Aug 1, 2026 · last used just now",
                               trail='<span class="badge b-pos">Current</span>', tone=7),
                ])
                + seclabel("Other sessions", right="2")
                + listcard([
                    person_row(None, "MacBook · Chrome",
                               "Karachi · signed in Jul 22, 2026 · last used yesterday",
                               trail='<span class="btn sm out">Revoke</span>', tone=7),
                    person_row(None, "Pixel · Chrome",
                               "Lahore · signed in Jun 2, 2026 · expires Sep 1, 2026",
                               trail='<span class="btn sm out">Revoke</span>', tone=7),
                ])
                + '<div style="padding:16px"><div class="btn dstr-out">Revoke all other '
                'sessions</div></div>'
                + '<p class="t-cap c-sec" style="margin:-4px 20px 0">Location is inferred from the '
                'network address and can be wrong. If you do not recognise a session, revoke it '
    ...
    write("34-account.html", "34", "Account, profile, security and notifications",
          ...
          "its own setting: a default currency converts nothing, a password change ends other sessions, "
          ...
          [
              ...
              frame("Devices", " Every session carries when it started, when it was last used and "
                               "when it expires, with an honest caveat about location.",
                    phone(sessions)),
