...
from lib import (statusbar, homebar, nav, largetitle, tabbar, frame, phone as frame_phone,
                 write, avatar, display_symbol, IC, tick)
...
def phone(body, cls=""):
    return frame_phone(body, f"brand personal {cls}".strip())


def summary(iso, income, spending, net, net_kind="pos"):
    """Income / Spending / Net using the viewer's display symbol."""
    ...
    return card(
        ...
        f'<span class="t-body mono">{symbol}{income}</span></div>'
        f'<div class="rrow" style="display:flex;justify-content:space-between;'
        f'padding:6px 16px"><span class="t-body c-sec">Spending</span>'
        f'<span class="t-body mono">{symbol}{spending}</span></div>'
        ...
        f'<span class="mt mono {col}"><span class="iso">{symbol}</span>'
        f'<span class="v t-headline semi">{net}</span></span></div>')


TXNS_AUG = [
    expense_row("Salary", "Aug 1 · Income · Salary", "4,200.00", "USD"),
    expense_row("Rent", "Aug 1 · Utilities", "1,450.00", "USD"),
    expense_row("Groceries", "Aug 2 · Groceries · Corner store", "86.40", "USD"),
    expense_row("Coffee", "Aug 3 · Food & Drink", "4.75", "USD"),
]

TXNS_JUL = [
    expense_row("Salary", "Jul 1 · Income · Salary", "4,200.00", "USD"),
    expense_row("Rent", "Jul 1 · Utilities", "1,450.00", "USD"),
    expense_row("Ski hire", "Jul 28 · Entertainment", "180.00", "USD"),
]
...
def build():
    ...
    populated = dash(
        ctx_marker("personal")
        + seclabel("August 2026", first=True)
    ...
    overspent = dash(
        ctx_marker("personal")
        + seclabel("August 2026", first=True)
        + summary("USD", "0", "18,400", "18,400", net_kind="neg")
        + '<p class="t-cap c-sec" style="margin:-4px 20px 0">No income has been recorded this '
        'month, so everything spent is counted as overspending.</p>'
        + MODE
        + seclabel("Recent entries", right="View all")
        + listcard([
            expense_row("Lahore trip taxi", "Aug 2 · Transport", "3,400", "USD"),
            expense_row("Dinner", "Aug 1 · Food & Drink", "15,000", "USD"),
        ]))
    ...
    write("29-personal-dashboard.html", "29", "Personal dashboard", "P01",
          ...
          "the user is in. Amounts use the viewer's display symbol without conversion, and the net "
          "is a labelled phrase, never a signed number.",
          [
              ...
              frame("Overspent", " Same layout, opposite net, and it explains why a month with no "
                                  "income reads the way it does.", phone(overspent)),
              ...
              frame("Loading", " Skeleton keeps the context marker because it does not depend on the "
                               "data.", phone(loading)),
          ])

    # ------------------------------------------------------------------ P02 --
    def txlist(body, tail=""):
        return (statusbar() + nav(title="Transactions", back="Personal", trail=FILTER)
                + tail
                + f'<div class="scroll">{body}</div>' + homebar())

    months = txlist(
        seclabel("August 2026", right="4 entries", first=True) + listcard(TXNS_AUG)
        + seclabel("July 2026", right="3 entries") + listcard(TXNS_JUL))

    filtered = txlist(
        '<div style="display:flex;gap:8px;padding:4px 16px 0;flex-wrap:wrap">'
        '<span class="badge b-brand">Expenses ✕</span>'
        '<span class="badge b-brand">Groceries ✕</span></div>'
    ...
    noresults = txlist(
        '<div style="display:flex;gap:8px;padding:4px 16px 0;flex-wrap:wrap">'
        '<span class="badge b-brand">Healthcare ✕</span></div>'
        + empty(GLYPH["search"], "No entries with these filters",
                "There are 7 entries in August, but none use Healthcare. "
                "Clear the category filter to see all of them.", "Clear category filter"))

    write("30-personal-transactions.html", "30", "Personal transactions", "P02",
          "Grouped by month, with the viewer's Account symbol beside each amount. Changing that symbol "
          "does not convert or rewrite any transaction.",
          [
    ...
    dup_sheet = sheet(
        ...
        + card('<div class="facts" style="padding:12px 16px">'
               ...
               '<span class="t-body mono">$45.00</span></div>'
               '<div class="f"><span class="k t-body">Your share</span>'
               '<span class="t-body mono">$15.00</span></div>'
    ...
    detail_body = (
        ...
        '<span class="iso">$</span><span class="v">86.40</span></span>'
        ...
        + seclabel("Counted in")
        + card(kv_row("August 2026 spending", "$1,541.15")
               + kv_row("Groceries category", "$312.80")))
    ...
    delete = (statusbar() + nav(back="Transactions")
              ...
              '<span class="t-cap semi mono">$86.40</span></div>'
    ...
    def catrow(name, amount, pct, width):
        return (f'<div class="row catrow" style="display:block">'
                ...
                f'<span class="t-body mono">${amount}</span></div>'
    ...
    reports = (statusbar() + nav(title="Reports", back="Personal")
               ...
               + listcard([
                   catrow("Utilities", "1,450.00", 94, 94),
                   catrow("Groceries", "312.80", 20, 20),
                   catrow("Food & Drink", "96.55", 6, 6),
               ...
               + listcard([
                   kv_row("August 2026", "Left over $2,658.85", vcls="t-body mono"),
                   kv_row("July 2026", "Left over $1,987.20", vcls="t-body mono"),
                   kv_row("June 2026", "Overspent $214.60", vcls="t-body mono c-neg"),
               ])
    ...
    mode_sheet = sheet(
        ...
        + card('<div class="row"><span class="grow"><span class="t-body med">Cash out of pocket'
               ...
               'you paid toward shared expenses. Answers '
    ...
    write("33-personal-reports.html", "33", "Personal reports and report mode",
          "P08 · P09",
          "One period, chosen explicitly, with the viewer's display symbol on every amount. Every bar "
          "in the trend chart has its exact "
