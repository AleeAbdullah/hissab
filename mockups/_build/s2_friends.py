...
def friends_phone(body, cls=""):
    return phone(body, f"brand friends {cls}".strip())
...
def build():
    ...
    write("05-friends.html", "05", "Friends", "F01",
          ...
          [
              ...
              frame("200% text", " <em>Scaling archetype 1 of 3 — lists.</em> Type doubles, padding does "
                    ...
                    friends_phone(scaled, cls="x2 tall")),
          ])
    ...
    sent = ('<div class="scroll"><div class="success">'
            ...
            '</div><div class="brand-actions stack"><div class="btn sec">Done</div></div></div>')
    ...
    write("07-connection-requests.html", "07", "Connection requests", "F03 · F04",
          ...
          [
              frame("Pending", " Actions are on the row, full-width enough to clear 44 pt. Outgoing "
                               "gets Cancel only.", friends_phone(pending_view)),
    ...
    def ledger_screen(name, initials, tone, statement, iso, amount, direction,
        ...
        return (statusbar() + nav(back="Friends", trail=GEAR)
                + '<div class="scroll">'
                + f'<div class="brand-profile-head">'
                ...
                + f'<div class="card"><div class="brand-balance">{bal}</div></div>'
                + '<div class="brand-actions split">'
                + '<span class="btn">Add expense</span>'
                + (f'<span class="btn sec">Settle up</span>' if not settled else "")
                + '</div>'
                + ('<div class="brand-actions split stack">'
                   '<span class="btn out">Remind</span></div>' if direction == "Owes you" else "")
                + breakdown
    ...
    write("09-friend-safety.html", "09", "Friend safety, block, blocked list", "F06 · F07 · F08",
          ...
          [
              ...
              frame("Block confirmation", " Four consequences as facts, including the exact "
                                          ...
                                          "it does, not &ldquo;OK&rdquo;.", friends_phone(block_dialog)),
