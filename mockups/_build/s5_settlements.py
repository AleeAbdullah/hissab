...
from lib import statusbar, homebar, nav, frame, phone as frame_phone, write, avatar, IC, tick
...
def phone(body, cls=""):
    return frame_phone(body, f"brand settlements {cls}".strip())
...
def build():
    ...
    choose = (statusbar() + nav(title="Settle up", lead=CANCEL)
              ...
              + '<p class="t-cap c-sec" style="margin:0 20px 4px">Choose the relationship balance '
              'this external payment should change. Amounts use your Account display symbol.</p>'
              + seclabel("You owe", first=True)
              + listcard([
                  select_row("JD", "John Doe", "$187.50 · across 3 ledgers", True, tone=2, box=False),
                  select_row("OF", "Omar Farooq", "$12,500 · Lahore trip", False, tone=4, box=False),
              ])
              + seclabel("Owes you", right="You can record these too")
              + listcard([
                  select_row("PN", "Priya Nair", "$64.00 · Flat 3B", False, tone=3, box=False),
                  select_row("LT", "Lena Toure", "$3,200 · direct", False, tone=6, box=False),
              ])
              + '</div>'
    ...
    def payform(amount, out=None, empty_amt=False, direction="You paid John Doe"):
        return ('<div class="ctx"><span class="who t-supp">Payment · '
                ...
                + seclabel("Balance with John Doe")
    ...
    over_sheet = sheet(
        ...
        + card('<div class="facts" style="padding:12px 16px">'
               ...
               '<span class="t-body mono">$250.00</span></div>'
    ...
    success = (statusbar() + nav(title="Payment recorded")
               ...
               '<p class="t-supp c-sec" style="margin:8px 0 0">$100.00 to John Doe on '
               'Aug 3, 2026. John Doe can see the record in their ledger.</p></div>'
               + seclabel("Balance with John Doe", first=True)
               + card('<div class="row"><span class="grow"><span class="t-body med">'
                      'John Doe</span><span class="t-cap c-sec" style="display:block">'
                      'Was $187.50</span></span>'
                      '<span class="val"><span class="t-cap c-sec" style="display:block">'
                      'You owe</span><span class="t-body semi mono c-neg">$87.50</span>'
                      '</span></div>')
               + '</div>'
    ...
    write("24-record-settlement.html", "24", "Settle up and record a payment",
          ...
          "rather than in a footnote. One relationship balance produces one record, and the amount "
          "uses the viewer's display symbol without conversion. Overpayment is allowed but never silent.",
          [
              frame("Choose balance", " Both directions are offered, because recording a payment you "
                                      ...
                                      "the balance it will change.", phone(choose)),
    ...
    pay_detail_body = (
        ...
        + listcard([
            person_row("AR", "Alina Rehman (you)", "Paid",
                       trail='<span class="t-body mono">$100.00</span>', tone=6),
            person_row("JD", "John Doe", "Received",
                       trail='<span class="t-body mono">$100.00</span>', tone=2),
        ])
        ...
        'expense. It moves the whole balance with this person.</p>'
        + seclabel("History")
        + listcard([
    ...
    pay_delete = (statusbar() + nav(back="John Doe")
                  ...
                  '<span class="t-cap semi mono">$100.00</span></div>'
    ...
    remind_body = (
        ...
        '<span class="t-cap c-sec">Flat 3B</span></span></div>'
        + seclabel("What they will see in Hissab", first=True)
        + card('<div class="facts" style="padding:12px 16px">'
        ...
        + card(kv_row("Last reminder", "None sent")
               + kv_row("Allowed", "Once per 24 hours")))
    ...
    sent = (statusbar() + nav(title="Reminder sent")
            ...
            '<p class="t-supp c-sec" style="margin:8px 0 0">Priya Nair can now see the reminder in '
            'Hissab. You can send another after the 24-hour cooldown.</p></div></div>'
    ...
    unavailable = (statusbar() + nav(title="Send a reminder", lead=CANCEL)
                   + '<div class="scroll">'
                   + banner("Push reminders are off for Priya Nair",
                            "The reminder will still appear in Hissab. Their lock screen will not "
                            "receive a push notification.", kind="w")
                   + remind_body + '</div>'
                   + footer('<div class="btn">Send reminder</div>')
                   + homebar())
    ...
    write("26-reminders.html", "26", "Reminder review and result", "S05 · S06",
          ...
          [
              ...
              frame("Push off", " The reminder is still delivered in-app; only the lock-screen push is "
                                "suppressed.",
                    phone(unavailable)),
          ])
